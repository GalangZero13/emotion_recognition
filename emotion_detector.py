...
while True:
    ...
    rects = detector.detectMultiScale(gray, scaleFactor = 1.1, flags = cv2.CASCADE_SCALE_IMAGE)
    ...
    if len(rects) > 0 :
        ...
        #count for the inference time
        elapsed_time = time.time() - start_time
        print('inference time cost: {}'.format(elapsed_time))
        
        cv2.putText(frameClone, label, (fX, fY - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 0, 0), 2)
